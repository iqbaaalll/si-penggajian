...
class PayrollController extends Controller
{
    ...
    public function runPayroll(Request $request)
    {
        ...
        if ($savePayrollPeriod) {
            ...
            $carbonDate = Carbon::createFromFormat('Y-m', $payrollMonth);
            ...
            if ($carbonDate->month == 12) {
                foreach ($employees as $employee) {
                    ...
                    $brutoSalary = $basicSalary + $bonus + $thr;//gaji kotor des

                    $year = Carbon::now()->year;

                    $payrolls = Payroll::with('payrollPeriod')
                        ->whereHas('payrollPeriod', function ($query) use ($year) {
                            $query->whereYear('payrollMonth', $year);
                        })
                        ->get();

                    $brutoSalaryNov = $payrolls->filter(function ($payroll) {
                        $month = Carbon::parse($payroll->payrollPeriod->payrollMonth)->month;
                        return $month >= 1 && $month <= 11;
                    })->sum('brutoSalary');//hasil total gaji kotor jan-nov

                    $brutoSalaryYear  = $brutoSalaryNov + $brutoSalary; //menjumlahkan total gaji kotor jan-nov dengan gaji kotor des
                    $netoSalaryYear = $brutoSalaryYear - 6000000; //mengurangi total gaji kotor jan-des dengan biaya jabatan
                    ...
                    if ($ptkpStatus == "TK/0") {
                        $ptkpAmount = 54000000;
                    } elseif ($ptkpStatus == "TK/1" || $ptkpStatus == "K/0") {
                        $ptkpAmount = 58500000;
                    } elseif ($ptkpStatus == "TK/2" || $ptkpStatus == "K/1") {
                        $ptkpAmount = 63000000;
                    } elseif ($ptkpStatus == "TK/3" || $ptkpStatus == "K/2") {
                        $ptkpAmount = 67500000;
                    } elseif ($ptkpStatus == "K/3") {
                        $ptkpAmount = 72000000;
                    }

                    $pkpAmount = $netoSalaryYear - $ptkpAmount;
                    $taxRate = 0;

                    if ($pkpAmount <= 60000000) {
                        $taxRate = $pkpAmount * 0.05;
                    } elseif ($pkpAmount <= 250000000) {
                        $taxRate = (60000000 * 0.05) + (($pkpAmount - 60000000) * 0.15);
                    } elseif ($pkpAmount <= 500000000) {
                        $taxRate = (60000000 * 0.05) + ((250000000 - 60000000) * 0.15) + (($pkpAmount - 250000000) * 0.25);
                    } else {
                        $taxRate = (60000000 * 0.05) + ((250000000 - 60000000) * 0.15) + ((500000000 - 250000000) * 0.25) + (($pkpAmount - 500000000) * 0.30);
                    }

                    $taxAmountYear = $payrolls->filter(function ($payroll) {
                        $month = Carbon::parse($payroll->payrollPeriod->payrollMonth)->month;
                        return $month >= 1 && $month <= 11;
                    })->sum('taxAmount');//hasil total potongan pajak PPh21 jan-nov

                    $taxAmount = ($taxRate - $taxAmountYear);
